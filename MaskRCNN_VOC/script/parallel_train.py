...
# def worker(rank, world_size, torch_task, save_dir, device_list):
#     setup(rank, world_size)
#     TorchTaskWorker(torch_task).parallel_train_val_worker(rank = rank, world_size = world_size, main_rank=0, batch_size = 2, epoch = 30, device = device_list[rank],
#         # train val option
#         val_frequency = 1, 
#         show_log = True,
...
#         np.random.seed(0)
#         mp.spawn(worker,
#             args=(world_size, torch_task, save_dir, ["cuda:3"]),
#             nprocs=world_size,
#             join=True)
